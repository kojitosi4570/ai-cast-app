...
def call_gemini_chat_engine(system_instruction, chat_history):
    ...
    for i, h in enumerate(chat_history):
        ...
        if i == 0 and role == "model":
            contents.append({"role": "user", "parts": [{"text": "（あなたからいいね！を送りました）"}]})
...
def render_legal_documents():
    ...
    with col2:
        with st.expander("📄 利用規約"):
            ...
            **第2条（AI自動応答に関する同意）**
            1. ユーザーは、当サービス内のすべての対話相手がAIシステムによって自動生成された架空のメッセージであることに同意し、楽しむものとします。
...
def main():
    ...
    # 💾 セッション状態の初期化
    if "swipe_index" not in st.session_state: st.session_state.swipe_index = 0  
    if "last_matched_cast" not in st.session_state: st.session_state.last_matched_cast = None  
    if "current_tab" not in st.session_state: st.session_state.current_tab = "🔍 お相手探し"
    if "active_chat_cast_id" not in st.session_state: st.session_state.active_chat_cast_id = None 
        ...
        if stripe:
            try:
                ...
                if checkout_session.payment_status == "paid":
                    ...
                    st.success("🎉 お支払いが確認できました！プレミアム会員としてお喋りをお楽しみください！")
    ...
    if st.session_state.last_matched_cast:
        ...
        if not b64_cast_img: b64_cast_img = "https://placehold.co/150x150?text=AI+Cast"
    ...
    if st.session_state.current_tab == "🔍 お相手探し":
        ...
        if unmatched_casts_all:
            recommend_html = """<div style="display: flex; flex-direction: row; gap: 15px; overflow-x: auto; padding: 10px 0; -webkit-overflow-scrolling: touch; -webkit-tap-highlight-color: transparent;">"""
        ...
        with col_b1:
            # 🛡️ 解決：(user_id, cast_id) のバインドを100%完璧に行い、sqlite3のエラーを解消
