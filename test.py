...
# =====================================================================
# 3. 【ロジック層】定型文メッセージ（AI不使用時のバックアップ・（笑）完全排除）
# =====================================================================
...
def generate_base_second_message(user_name, char_type):
    return f"「{user_name}さんお仕事お疲れ様です！私、周りから『合法の人間ビーズクッション』って呼ばれるくらいモチモチで癒やし系らしいから、疲れてたら1回ダイブしてみる？最近ちゃんと夜眠れてますか？」"
...
if st.button("▶︎ AIチャットを生成する", type="primary"):
    ...
    for cast in casts[:4]:  
        ...
        base_prompt_2nd = (
            ...
            f"【出力形式】：ダラダラ説明せず、2〜3行の短いメッセージ本文のみを出力してください。\n\n"
        )
        ...
        if dice_2nd <= 30:
            ...
            prompt_2nd = base_prompt_2nd + (
                "【個別ミッション：王道の純粋癒やし】\n"
                "男が一番求めている心の疲れを包み込む、優しくてクリーンな精神的ケアを提供してください。\n"
                "相手の日常や仕事のがんばりを完璧に肯定し、「本当にいつもお疲れ様、無理しないでね」というニュアンスをあなたの口調で自然に伝えてください。"
            )
        elif dice_2nd <= 60:
            # ② おちゃめな匂わせ（30%）
            prompt_2nd = base_prompt_2nd + (
                "【個別ミッション：おちゃめな匂わせ短文】\n"
                "「合法の人間ビーズクッション」「プレミアム低反発ひざ枕」など、男が勝手にフィジカルな想像（密着感や柔らかさ）をして突っ込んでしまうようなお茶目なパワーワード（例え話）を1つだけ混ぜて短文で作ってください。"
            )
        elif dice_2nd <= 90:
            # ③ ツッコミ待ち大ボケ（30%）
            prompt_2nd = base_prompt_2nd + (
                "【個別ミッション：ツッコミ待ち大ボケ】\n"
                "少し言葉遣いを崩して「半端ねぇマウンテン級の癒やしパワー」や「たまんねぇ癒やし」「やべぇ」といった、インパクトが強すぎて男が思わずツッコミたくなるような男勝りでお茶目なボケ文面を短文で作ってください。"
            )
        else:
            # ④ 異次元のヤバいボケ（10%）
            prompt_2nd = base_prompt_2nd + (
                "【個別ミッション：異次元のヤバいボケ】\n"
                "「ゲルゲロパーレー」「バブバブドカン」など、現実には存在しない、意味がわからない全く新しい造語・やばい言葉を1つ自分で作り出し、「私の〇〇（造語）で癒やしてあげる」という風に、急に電波系というかやばい電波を受信した女の子のような狂気的なボケを真顔でぶち込んでください。"
            )
